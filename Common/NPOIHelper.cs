...
public class NPOIHelper
{
    ...
    /// 导出数据到本地
    /// </summary>
    /// <param name="dtSource">要导出的数据源</param>
    /// <param name="file">返回第一次保存的路径</param>
    /// <param name="strHeaderText">表头文本(追加导出时候赋空值否则必须赋值表头文件名)</param>
    public static void ExportToLocal(DataTable dtSource, out string file,out string error, string strHeaderText="")
    {
        file = "";
        error = "";
        try
        {
            ...
            {
                ExportDTtoExcel(dtSource, strHeaderText, fileDialog.FileName);
    ...
    public static void ExportDt(DataTable dt, string tableName, string fileName)
    {
        HSSFWorkbook workbook = new HSSFWorkbook();
        ISheet sheet = string.IsNullOrEmpty(dt.TableName) ? workbook.CreateSheet(tableName) : workbook.CreateSheet(dt.TableName);//创建工作表
        ...
        for (int i = 1; i <= dt.Rows.Count; i++)//遍历DataTable行
        {
            ...
            for (int j = 0; j < dt.Columns.Count; j++)//遍历DataTable列
            {
                ...
                cell.SetCellValue(dataRow[j].ToString().Replace("00:00:00",""));//设置列的内容	 
            }
        }
        #endregion
    ...
    /// <summary>
    ///用DEV控件导出函数导出
    ///<param name="fileNmae">要保存的文件名</param>
    ///<param name="gridControl">导出数据对应的列表控件</param>
    /// </summary>
    //public static void Export_Dev(string fileNmae, GridControl gridControl)
    //{
    //    try
    //    {
    //        SaveFileDialog fileDialog = new SaveFileDialog();
    //        fileDialog.Filter = "Excel文件(*.xls)|*.xls|Excel文件(*.xlsx)|*.xlsx;";
    //        fileDialog.FileName = fileNmae;
    //        if (fileDialog.ShowDialog() == DialogResult.OK)
    //        {
    //            string hzm = fileDialog.FileName.Substring(fileDialog.FileName.LastIndexOf(".") + 1);
    //            if (hzm == "xls")
    //            {
    ...
    //            }
    //            ClientHelper.ShowInfo("操作成功!", "操作提示");
    //        }
    //    }
    //    catch (Exception ex)
    //    {
    //        ClientHelper.ShowInfo(ex.Message);
    //    }
    //}
    ...
    public static void ExportToLocal(out string file, out  string error, DataTable dtSource)
    {
        file = "";
        error = "";
        try
        {
            ...
            if (fileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                //写入一个空的文件
                ExportDTtoExcel(dtSource, "", fileDialog.FileName);
            }
            else
            {
                error = "您已取消导出操作";
    ...
    static MemoryStream ExportDT(DataTable dtSource, string strHeaderText,string secondText)
    {
        HSSFWorkbook workbook = new HSSFWorkbook();
        HSSFSheet sheet = workbook.CreateSheet() as HSSFSheet;
        ...
        foreach (DataRow row in dtSource.Rows)
        {
            ...
            if (rowIndex == 65535 || rowIndex == 0)
            {
                ...
                //    HSSFRow headerRow = sheet.CreateRow(1) as HSSFRow;
                //    headerRow.HeightInPoints = 25;
                //    headerRow.CreateCell(0).SetCellValue(secondText);
    ...
    /// <param name="strHeaderText">表头文本</param>
    /// <param name="strFileName">保存位置</param>
    public static void ExportDTtoExcel(DataTable dtSource, string strHeaderText, string secondText, string strFileName)
    {
        string[] temp = strFileName.Split('.');

        if (temp[temp.Length - 1] == "xls" && dtSource.Columns.Count < 256 && dtSource.Rows.Count < 65536)
        {
            using (MemoryStream ms = ExportDT(dtSource, strHeaderText, secondText))
            {
                using (FileStream fs = new FileStream(strFileName, FileMode.Create, FileAccess.Write))
                {
                    byte[] data = ms.ToArray();
    ...
    static DataTable ImportDt(ISheet sheet, int HeaderRowIndex, bool needHeader)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                for (int i = headerRow.FirstCellNum; i <= cellCount; i++)
                {
                    object cellValue = headerRow.GetCell(i);
                    if (cellValue == null)
                    {
                    ...
                    else if (table.Columns.IndexOf(cellValue.ToString()) > 0)
                    {
                        DataColumn column = new DataColumn(Convert.ToString("重复列名" + i));
                        table.Columns.Add(column);
                    }
    ...
    /// 将数据追加到现有Sheet中
    /// </summary>
    /// <param name="outputFile">文件路径</param>
    /// <param name="sheetname">要追加的Sheet名称，如果不存在则自动创建</param>
    /// <param name="dt"></param>
    public static void AppendToSheet(string outputFile, string sheetname, DataTable dt)
    {
    ...
    public static MemoryStream Export(DataTable dtSource, string strHeaderText)
    {
        ...
        #region 右击文件 属性信息
        {
            ...
            //si.CreateDateTime = TimeService.ServiceTime;
            workbook.SummaryInformation = si;
        }
        #endregion
