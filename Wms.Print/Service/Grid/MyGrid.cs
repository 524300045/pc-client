...
namespace Wms.Print.Service.Grid
{
    ...
    /// 
    /// </summary>
    public class MyGrid : GridBase
    {
        #region	mArrStrGrid相关的属性与方法 GridText获取设置mArrStrGrid、SetText(int row,int col,string text)、GetText(int row,int col)获取单元格文本
        ...
        {
            mArrStrGrid[RowSel, ColSel] = text;
        }
        ...
        {
            return mArrStrGrid[RowSel, ColSel];
        }

        #endregion

        private string mColsAlignList = "";				//列对齐列表,由Left，Center，Right第一个字母组成的串的对齐方式的串
        ...
        public string ColsAlignString
        {
            get
            {
                return this.mColsAlignList;
            }
            set
            {
                if (value != null)
                {
                    this.mColsAlignList = value;
                }
            }
        }
        ...
        //重要属性									
        private object mDataSource = null;				//△会改变行列及其它属性
        #region DataSource
        /// <summary>
        /// 获取或设置数据源，可以是任意的二维数组型的，如二维数组、网格等
        /// </summary>
                ...
                if (value != null)
                {
                    ...
                    switch (value.GetType().ToString())
                    {
                        ...
                        case "System.String[]":			//一维数组
                            string[] arr1 = (String[])mDataSource;
                        ...
                        case "System.String[,]":		//二维数组
                            this.GridText = (String[,])mDataSource;
                            break;
                        case "System.Data.DataTable":	//数据表格							
                            this.GridText = ToArrFromDataTable((DataTable)mDataSource);
                            break;
                        case "System.Windows.Forms.DataGrid":
                            this.GridText = ToArrFromDataGrid((DataGrid)mDataSource);
                            break;
                    }
        ...
        {
            return GetMergeCell(this.Location, mArrStrGrid, this.PreferredRowHeight, this.ColsWidth, RowSel, ColSel);
        }
        ...
        private void InitColsAlignString()
        {
            ...
            for (int i = mColsAlignList.Length; i < this.Cols; i++)
            {
                malignString += malignChar;
            }
            this.mColsAlignList = malignString;

            if (mColsAlignList.Length > Cols)
            {
                mColsAlignList.Substring(0, Cols);
            }
        }
        ...
        /// <param name="source"></param>
        /// <returns></returns>
        public string[,] ToArrFromDataGrid(DataGrid source)
        {
        ...
        /// <param name="source"></param>
        /// <returns></returns>
        //        public string[,] ToArrFromHtmlTable(HtmlTable source)
        //        {
        //            if (source == null)
        //            {
        //                return new string[0, 0];
